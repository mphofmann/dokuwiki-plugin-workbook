...
class AdmincoreWbTool {
    ...
    public static function Reset($inWb, $inTool) {
        if (empty($inWb) or empty($inTool)) return base\BaseXhtmlMsg::Echo('Warning', __METHOD__, "$inWb:$inTool", "Missing inputs.");
        if (!mod\ModWbTool::CommandEnabledCheck($inWb, $inTool, __FUNCTION__)) return base\BaseXhtmlMsg::Echo('Warning', __METHOD__, "$inWb:$inTool", "Method '" . __FUNCTION__ . "' not enabled.");
    ...
    public static function Syncclear($inWb, $inTool) {
        ...
        base\BaseXhtmlMsg::Echo('Info', __METHOD__, "$inWb:$inTool", "Sync cleared.");
    }
    /* -------------------------------------------------------------------- */
    ...
    public static function Resync($inWb, $inTool) {
        ...
        if (!sys\SysRemote::EnabledCheck('login')) {
            base\BaseXhtmlMsg::Echo('Warning', __METHOD__, '', 'Marketplace login failed.');
            ...
            base\BaseXhtmlMsg::Echo('Notice', __METHOD__, "$inWb:$inTool", "ACL check failed. [$acl]");
        } else {
    ...
    public static function Clear($inWb, $inTool) {
        if (empty($inWb) or empty($inTool)) return base\BaseXhtmlMsg::Echo('Warning', __METHOD__, "$inWb:$inTool", "Missing inputs.");
        if (!mod\ModWbTool::CommandEnabledCheck($inWb, $inTool, __FUNCTION__)) return base\BaseXhtmlMsg::Echo('Warning', __METHOD__, "$inWb:$inTool", "Method '" . __FUNCTION__ . "' not enabled.");
        if ($inTool != 'ztrash') return base\BaseXhtmlMsg::Echo('Warning', __METHOD__, "$inWb:$inTool", "Only ZTrash can be cleared.");
        base\BaseXhtmlMsg::Echo('Info', __METHOD__, "$inWb:$inTool", '');
