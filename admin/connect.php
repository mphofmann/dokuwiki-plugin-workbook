        ...
        foreach (['plugin' => 'Plugins (lib/plugins)', 'template' => 'Templates (lib/tpl)'] as $exttype => $th) {
            ...
            if (is_array($ar[$exttype . 's'])) {
                foreach ($ar[$exttype . 's'] as $id => $val) {
                    ...
                    $returns[] = [$id, admin\AdminCmd::ExecGet("dokuadmin\DokuadminInstall::ExtensionAction action=note type=$exttype id=$id url=$url"), admin\AdminCmd::ExecGet("dokuadmin\DokuadminInstall::ExtensionAction action=status type=$exttype id=$id url=$url") . $note, admin\AdminXhtml::ButtonGet("dokuadmin\DokuadminInstall::ExtensionAction action=install type=$exttype id=$id url=$url tarpath=$tarpath", 'install', $attr), admin\AdminXhtml::LinkGet("?do=admin&page=extension&tab=$exttype")];
                }
            }
        }
        // Webroots
